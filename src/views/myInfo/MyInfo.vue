          ...
          <div class="col-lg-12 col-sm-6 col-12 main-section text-center">
            ...
            <div class="row user-detail">
              <div class="col-lg-12 col-sm-12 col-12">
                ...
                <span>{{
                  userEmail ? "(" + userEmail + ")" : "test@google.com"
                }}</span>
                <br />
      ...
      <div class="col-md">
        <div class="row">
          <div style="text-align : center;">
            <span>
              ...
              <span
                >{{ userName ? userName + " 님" : "홍길동" }} 님의 월렛 내의
                자산이 2019년 11월 28일,</span
              >
              <br />
        ...
        <div class="row">
          <div style="text-align : center;">
            <span
              >* ※ 당월 고객님의 자산은 다음과 같이
              <span style="color:red;">증감</span>하였으며<br />실제 거래량과
              차이가 있을 수 있습니다.</span
            >
          </div>
        </div>
        ...
        <tbody>
          <tr v-for="item in transactionList" :key="item.guid">
            ...
            <td style="text-align : right;">
              {{ fn_stringByMoney(item.amount) }}
            </td>
            <td style="text-align : right;">
              {{ fn_stringByMoney(item.balance) }}
            </td>
...
export default {
  ...
  methods: {
    async fn_userInfo() {
      let userInfoArr = this.$store.getters.getUserInfo;
      var vm = this;

      if (userInfoArr[0]) {
        alert("true : " + userInfoArr[0]);
      } else {
        alert("로그인이 필요한 서비스 입니다. 로그인 해주세요.");
        this.$router.push("/login");
      }

      // await userInfoApi(phone).then(
      //   function(response) {
      ...
      var resultData = response.data.result.list[0];
      vm.userName = resultData.fullname;
      vm.userEmail = resultData.email;
      vm.userPhone = resultData.phoneNo;
      ...
      http
        ...
        .then(response => {
          ...
          for (let i = 0; i < this.transactionList.length; i++) {
            this.transactionList[i].updateDt = this.transactionList[
              i
            ].updateDt.replace("T", " ");
          }
